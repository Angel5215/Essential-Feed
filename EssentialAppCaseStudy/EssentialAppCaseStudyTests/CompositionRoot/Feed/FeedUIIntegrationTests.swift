...
final class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadFeedActions_requestFeedFromLoader() async {
        ...
        sut.simulateUserInitiatedReload()
        XCTAssertEqual(loader.loadFeedCallCount, 3, "Expected a third request once the user initiates another load")
    }
    ...
    func test_tapOnErrorView_hidesErrorMessage() async {
        ...
        await loader.completeFeedLoadingWithError()
        XCTAssertEqual(sut.errorMessage, loadError)
    ...
    func test_feedImageView_loadsImageURLWhenVisible() async throws {
        let image0 = try makeImage(url: XCTUnwrap(URL(string: "https://url-0.com")))
        let image1 = try makeImage(url: XCTUnwrap(URL(string: "https://url-1.com")))
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_cancelsImageLoadingWhenNotVisibleAnymore() async throws {
        ...
        XCTAssertEqual(loader.cancelledImageURLs, [image0.url, image1.url], "Expected two cancelled image requests once second image is also not visible anymore")
    }
    ...
    func test_feedImageView_configuresViewCorrectlyWhenTransitioningFromNearVisibleToVisibleWhileStillPreloadingImage() async throws {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() async throws {
        let (sut, loader) = makeSUT()
        sut.simulateAppearance()
        await loader.completeFeedLoading(with: [makeImage(), makeImage()])
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, false, "Expected no retry action state change for second view once first imge loading completes successfully")
    ...
    func test_feedImageView_doesNotRenderLoadedImageWhenNotVisibleAnymore() async {
        ...
        await loader.completeImageLoading(with: anyImageData(), at: 0)

        XCTAssertNil(view?.renderedImage, "Expected no rendered image when an image load finishes after the view is not visible anymore")
    }

    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() async throws {
        let image = try makeImage(url: XCTUnwrap(URL(string: "https://url-0.com")))
        let (sut, loader) = makeSUT()
